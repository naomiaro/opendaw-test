...
- **[Task 9 — integration round] Barrier consistency between loop-recording and useRecordingSession:** the demo's hand-rolled barrier and the shared hook diverge on error wording ("A take failed to finalize:" vs "Recording finalization failed:"), timeout wording, and timeout duration (10_000 unnamed literal in loop-recording-demo.tsx:504 vs FINALIZATION_TIMEOUT_MS = 30_000 in useRecordingSession.ts:35). Reconcile: name the demo's constant, converge wording, decide one duration (30s likely right — 10s may false-positive on long multi-take finalizations). Cheap; apply in the integration round.
- **[Task 9 — integration round] useTakeDiscovery exposes raw refs (interface divergence):** the hook returns `pointerHubSubsRef`/`sampleLoadersRef` and loop-recording-demo.tsx:291-310 reaches in to terminate/reassign/snapshot them, while the sibling hooks (useTapePeaks, useRecordingSession) encapsulate lifecycle behind methods. The demo's hand-rolled ~55-line finalization barrier duplicates useRecordingSession's. Candidates: `terminateDiscovery()` + `snapshotLoaders()` methods, or fold the barrier into a shared shape. Pair with the barrier wording/timeout reconciliation item above.
- **[Task 9 — integration round] Page-shell duplication:** the Theme→CONSOLE_STYLES→GitHubCorner/BackLink→mc-kicker/mc-title/mc-intro→MoisesLogo scaffold is verbatim in loop-recording-demo.tsx:478-506 and recording-api-react-demo.tsx:314-331, and becomes 3x after Task 7. Candidate: `<ConsolePageShell>` wrapper (decide in integration round — campaign forbids restyling the shared site shell, but a wave-local wrapper for the wave's own pages is fair game).
- **[Task 9 — integration round] Code-block style duplication:** LoopRecordingReference.tsx:4-13 `codeStyle` re-implements inline code-block styling that also exists in midi-recording-demo.tsx and mixer-groups-demo.tsx. Candidate: shared constant in consoleTheme.ts (3-4 consumers after Task 7).
- **[Task 9 — note] TakeTimeline now depends on --mc-* vars resolving (7 inline-style usages) while living in shared src/components/; sole consumer injects CONSOLE_STYLES today. Either move it to src/demos/recording/ or document the dependency — decide in integration round.
