...
- [ ] **Step 2: Upstream contribution drafts** — two tracks, both STOP-for-user-review (nothing pushed to any remote or posted upstream without explicit approval):
  - **(a) Upstream fix PR (user-requested, conditional on the Task 7 verdict passing):** port the verified recording timing-alignment fixes onto current upstream openDAW `main` in the local upstream checkout (new branch; the candidate base is older than upstream HEAD, so port and re-verify compile), scrubbed of every origin identifier; draft the PR description as `debug/drafts/pr-recording-start-alignment.md` — measured signatures, repro page URL, register link. The "no suggested-fix" convention is issue-body-scoped and does not apply to the PR vehicle.
  - **(b) Issue drafts** for confirmed findings the PR does not fix (e.g. the loop-wrap finalization hang if unfixed on the candidate build): one md file per defect under `debug/drafts/`, live repro page URL + register link + measured signature, cause description WITHOUT a suggested-fix section, no origin naming.
