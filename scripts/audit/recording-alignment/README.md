...
Three scripts have their full output persisted beside the artifacts, in
`.verify-output/task12a-keepalive-classification.txt`,
`.verify-output/task12b-calibration-tables.txt` and
`.verify-output/task12c-real-input-tables.txt`. `.verify-output/` is gitignored, so
