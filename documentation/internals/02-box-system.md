...
`dependenciesOf()` (`graph.ts:355`) walks the graph from a starting set and collects every reachable box and pointer field. It honours `ResourceType`:

- Across an `"internal"` boundary: follow.
- Across a `"shared"` boundary: stop (don't include the shared target's children).
- Across a `"preserved"` boundary: follow but mark target UUID as "keep".

This is the foundation of copy/paste and clip cloning — every time openDAW duplicates state, this is how it decides what gets cloned versus shared.
