...
3. **Idempotency check before detection.** Both the SDK's `toTimeStretch` and the demo's `ensureTransientMarkers` test `file.transients.length() === 0` before calling the worker. Re-detection produces identical results but wastes seconds on long files.
