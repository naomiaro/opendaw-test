...
To reproduce manually without the static-setup button, use the debug page's own
file-drop flow (the listed Comp Lanes demo that previously served this purpose was
retired in favor of the Quick Swipe Comping demo): drop two distinct audio files,
switch to splice mode (splices are always exact-boundary), add a comp boundary, play.
