...
- [shared-source-double-process.md](./shared-source-double-process.md) — Audible amplitude artifact (snap or dip, depending on whether the seam is touching or overlapping) when two same-track regions reference the **same** `AudioFileBox`. `TapeDeviceProcessor` keys voices by `sourceUuid` and calls `voice.process` per adapter; the shared voice's `readPosition` advances `2 × bpn` in any block that contains both adapters. Demo: _pending_, audio fixture: `public/audio/test-440hz.wav`.
- [voice-fadein-clip-fadein-product.md](./voice-fadein-clip-fadein-product.md) — Audible ~−1.16 dB amplitude dip on the new voice's fade-in side of a crossfade between regions with **different** `sourceUuid`s. `PitchVoice` enters new voices in `Fading`/`fadeDirection=1` for 20 ms and multiplies the voice fade with the clip's gain buffer, turning a linear clip fade-in into a quadratic ramp over the first 20 ms. Demo: _pending_, audio fixtures: `public/audio/test-440hz.wav`, `public/audio/test-440hz-offset30.wav`.
